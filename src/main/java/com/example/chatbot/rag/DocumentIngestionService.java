...
import com.example.chatbot.repository.DocumentChunkRepository;
...
public class DocumentIngestionService {
    ...
    /**
     * Synchronous ingestion: extract -> chunk -> embed (batched) -> store.
     * For large files or high upload volume, move this to an async job queue
     * (e.g. a Spring @Async method or a proper task queue) and let the client
     * poll DocumentView.status - the schema (PROCESSING/READY/FAILED) already
     * supports that; this method just runs it inline for simplicity here.
     */
    @Transactional
    public DocumentView ingest(User user, MultipartFile file) {
        ...
        try {
            ...
            // Embed in one batched call per document (cheaper/faster than per-chunk calls)
            List<float[]> embeddings = embeddingClient.embed(chunkTexts, "document");
            ...
            for (int i = 0; i < chunkTexts.size(); i++) {
                ...
                vectorSearchRepository.saveEmbedding(chunk.getId(), embeddings.get(i));
            }
    ...
    public void deleteDocument(User user, UUID documentId) {
        ...
        chunkRepository.deleteByDocumentId(document.getId()); // cascades cover this too, but explicit is clearer
